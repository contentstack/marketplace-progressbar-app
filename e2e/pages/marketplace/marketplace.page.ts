...
export class Marketplace {
  ...
  await this.page.locator('[data-test-id="apps-progress-bar-modal-install-app"]').waitFor();
  await this.page.locator('[data-test-id="apps-progress-bar-modal-install-app"]').click();
  ...
  await this.page.getByRole('textbox', { name: 'cs-select-aria' }).fill(stackName);
  ...
  async unInstallApp(appName: string) {
    await this.page
      .locator('[data-test-id="apps-workflow-board-installed-on-tab"]')
      .waitFor();
    await this.page
      .locator(".Table__body__column > .SkeletonTile__svg")
      .first()
      .waitFor();
